...
**解** 取 $f(x)=x^2$，$g(x)=e^x$，列出：

$$
\begin{array}{ccl}
f(x)\text{ 及其各阶导数}&&g(x)\text{ 及其各次积分}\\
x^2&\overset{+}{\searrow}&e^x\\
2x&\overset{-}{\searrow}&e^x\\
2&\overset{+}{\searrow}&e^x\\
0&&e^x
\end{array}
$$

按箭头上方的运算符号，将箭头连接的函数相乘并合并，得到
...
计算机代数系统的一项强大功能是进行符号积分。这通过各系统指定的积分命令实现，例如 Maple 中的 `int` 和 Mathematica 中的 `Integrate`。

**例 5** 假设要计算函数 $f(x)=x^2\sqrt{a^2+x^2}$ 的不定积分。使用 Maple 时，先定义或命名函数：

```maple
f := x^2 * sqrt(a^2 + x^2);
```

然后对 $f$ 使用积分命令，并指定积分变量：
...
$$
\frac{1}{4}x(a^2+x^2)^{3/2}-\frac{1}{8}a^2x\sqrt{a^2+x^2}
-\frac{1}{8}a^4\ln\left(x+\sqrt{a^2+x^2}\right).
$$
...
$$
\frac{1}{8}a^2x\sqrt{a^2+x^2}+\frac{1}{4}x^3\sqrt{a^2+x^2}
-\frac{1}{8}a^4\ln\left(x+\sqrt{a^2+x^2}\right).
$$

如果要求 $0\le x\le\pi/2$ 上的定积分，可以使用格式
...
$$
\begin{aligned}
&\frac{1}{64}\pi(4a^2+\pi^2)^{3/2}
-\frac{1}{32}a^2\pi\sqrt{4a^2+\pi^2}+\frac{1}{8}a^4\ln2\\
&\qquad-\frac{1}{8}a^4\ln\left(\pi+\sqrt{4a^2+\pi^2}\right)
+\frac{1}{16}a^4\ln(a^2).
\end{aligned}
$$

也可以对常数 $a$ 的某个特定值计算定积分：
...
**例 6** 用 CAS 求 $\displaystyle\int\sin^2x\cos^3x\,dx$。
...
不同计算机代数系统处理积分的方式不同。例 5 和例 6 使用了 Maple，Mathematica 会给出略有不同的结果：

1. 对例 5，输入

   ```mathematica
   Integrate[x^2 * Sqrt[a^2 + x^2], x]
   ```

   Mathematica 返回

   $$
   \sqrt{a^2+x^2}\left(\frac{a^2x}{8}+\frac{x^3}{4}\right)
   -\frac{1}{8}a^4\operatorname{Log}\left[x+\sqrt{a^2+x^2}\right],
   $$

   不必再化简中间结果。该答案接近积分表中的公式 22。
...
2. 对例 6 的积分，输入
   ...
   $$
   \frac{\operatorname{Sin}[x]}{8}-\frac{1}{48}\operatorname{Sin}[3x]-\frac{1}{80}\operatorname{Sin}[5x],
   $$

   与 Maple 的答案不同，但两个答案都正确。

CAS 虽然十分强大，能够帮助解决困难问题，但每种系统都有局限。有时 CAS 甚至会使问题更复杂，也就是说给出极难使用或解释的答案。还要注意，Maple 和 Mathematica 都没有返回任意常数 $+C$。另一方面，稍作数学思考，可能就能把问题化成很容易处理的形式。习题 67 给出了一个例子。

许多硬件设备都能使用基于 Maple 或 Mathematica 等软件的积分应用程序。这些程序允许输入被积函数的符号表达式，再输出不定积分的符号表达式。许多这类软件应用也能计算定积分。除查积分表外，它们提供了另一种求积分的工具。不过，有时积分软件可能根本无法输出答案。
...
计算机代数系统可以计算许多收敛的反常积分。用 Maple 计算收敛积分
...
Maple 返回答案 $-\frac{1}{2}\pi+\ln5+\arctan2$。为得到数值结果，用求值命令 `evalf` 并指定有效数字位数：

```maple
evalf(%, 6);
```

符号 `%` 指示计算机对屏幕上最后一个表达式求值，此处即 $-\pi/2+\ln5+\arctan2$。Maple 返回 1.14579。

使用 Mathematica，输入

```mathematica
Integrate[(x + 3)/((x - 1)(x^2 + 1)), {x, 2, Infinity}]
```

返回 $-\pi/2+\operatorname{ArcTan}[2]+\operatorname{Log}[5]$。为得到六位有效数字的数值结果，使用命令 `N[%, 6]`，同样得到 1.14579。
...
**历史人物简介** 卡尔·魏尔斯特拉斯（Karl Weierstrass，1815–1897）。
...
| $b$ | $\displaystyle\int_1^b\frac{1-e^{-x}}{x}\,dx$ |
|---|---|
| 2 | 0.5226637569 |
| 5 | 1.3912002736 |
| 10 | 2.0832053156 |
...
\text{结果}&HHH&HHT&HTH&HTT&THH&THT&TTH&TTT\\
X&3&2&2&1&2&1&1&0
\end{array}
$$
...
<div align="center">
  <img src="images/8-22.png" alt="图 8.22 概率密度曲线下的面积表示概率。">
  <div style="font-style: italic; font-size: 0.92em; color: #666; margin-top: 0.35em;">图 8.22 连续随机变量 $X$ 的一个概率密度函数。</div>
</div>
